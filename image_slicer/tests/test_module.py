...
"""
Unit tests for the entire module.
"""

import unittest
import os
from image_slicer import get_columns_rows, calc_columns_rows
from image_slicer import save_tiles, slice, join
from PIL import Image


TEST_IMAGE = os.path.join(os.path.dirname(__file__), "data/sample.jpg")
TEST_DIR = os.path.join(os.path.dirname(__file__), "data/test_output")
NUMBER_TILES = 20
...
class GeneralTest(unittest.TestCase):
    def test_get_columns_rows_filenames(self):
        filenames = ["abc_01_01.jpg", "abc_01_02.jpg", "abc_02_01.jpg", "abc_02_02.jpg"]
...
class SplitTest(unittest.TestCase):
    ...
    def test_tiles_are_images(self):
        self.assertTrue(
            all(tile.image.__class__.__name__ == "_ImageCrop") for tile in self.tiles
        )
...
class IntegrationTest(unittest.TestCase):
    ...
    def test_image_size_equality(self):
        width_difference = abs(self.original.size[0] - self.reconstituted.size[0])
        height_difference = abs(self.original.size[1] - self.reconstituted.size[1])
        THRESHOLD = 2  # Error margin (px).
        self.assertTrue(
            width_difference <= THRESHOLD,
            "Width is {0} pixels out".format(width_difference),
        )
        self.assertTrue(
            height_difference <= THRESHOLD,
    ...
    def test_image_similarity(self):
        ...
        bad_pixels = len(
            set(self.original.histogram()) ^ set(self.reconstituted.histogram())
        )
